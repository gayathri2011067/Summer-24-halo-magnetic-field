...
module physical_grid
  ...
  integer, parameter :: nxphys = 51 !Resolution in z for physical grid
  ...
  integer, parameter :: nx = nxphys + 2*nxghost + 2*nxvacuum  !Resolution in z !CHANGED!
...
module make_a_grid
  ...
  subroutine construct_grid
    ...
    double precision, parameter :: len = 20.*h  !CHANGED!
    double precision, dimension(nx) :: spac

    dx=len/(nxphys+nxvacuum-1)  !x corresponds to z !CHANGED!
    alp=dt/dx**2  !alpha for finite difference scheme
    beta= dt/dx !beta for fin diff 

    do i=1,nx
      x(i)= -(10.*h +nxghost*dx) +(i-1)*dx +0.01 !NOTE:+0.01 to avoid 0 !CHANGED!
